...
async function getWeatherByCity() {
  ...
  const geoCodingResponse = await fetch(`https://nominatim.openstreetmap.org/search?city=${city}&format=json`);
...
async function getWeatherByCoordinates() {
  ...
  if (lat < -90 || lat > 90 || lon < -180 || lon > 180) {
    alert("Please enter valid coordinates within the range:\nLatitude (-90 to 90)\nLongitude (-180 to 180)");
    return;
  }

  const geoCodingResponse = await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lon}&format=json`);
  const geoCodingData = await geoCodingResponse.json();
